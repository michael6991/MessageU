...
* newer method is used in the function
*/
...
size_t Client::getFileMsg(Message* msg)
{
	...
	// create a path for the file according to temp directory.
	// the filename is the message id.
	std::string path = tempDir.string();
	path += "\\" + std::to_string(msg->msgID);


	// print absolute path of file to be save in client's tmp memory dir
	std::cout << "Content:" << std::endl;
	std::cout << boost::filesystem::absolute(path);
	std::cout << std::endl;
